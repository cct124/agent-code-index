...
如果要跑更严格的代码定位版基准，只保留最像真实工程排障的 code-location 问题，可以显式指定：

```bash
corepack yarn quality:run --env-file .env.development --queries-file test/quality/queriesv4.json --label qwen-v4 --repository-id agent-code-index --root-path /home/janex/project/ai-agent/agent-code-index --embedding-batch-size 16 --embedding-concurrency 8
corepack yarn quality:run --env-file .env.development.voyageai --queries-file test/quality/queriesv4.json --label voyage-v4 --repository-id agent-code-index --root-path /home/janex/project/ai-agent/agent-code-index --embedding-batch-size 16 --embedding-concurrency 8
corepack yarn quality:compare --left test/quality/results/qwen-v4.json --right test/quality/results/voyage-v4.json --output-file test/quality/results/qwen-v4-vs-voyage-v4.md
```
...
4. `queriesv4.json` 在 `v3` 基础上进一步收紧，只保留真实代码定位任务，并明确要求优先返回实现与测试代码，减少设计文档型查询。
5. 评测脚本会自动把整个 `test/quality` 目录加入本次索引忽略列表，避免查询文本、结果文件和评测脚本本身污染召回结果。
6. `--label` 决定默认输出文件名，首次会写到 `test/quality/results/<label>.json`；如果同名文件已存在，脚本会自动追加 UTC 时间戳，避免覆盖旧结果。
7. `quality:compare` 现在也默认保留历史报告；当目标 Markdown 已存在时，会自动生成带时间戳的新文件。
8. 只有显式传入 `--allow-overwrite` 时，脚本才会覆盖已有结果文件或报告。
9. 对比报告顶部会额外输出聚合指标，例如平均 chunk overlap、平均 file overlap、Top-3 文件重合度，以及 implementation/test/documentation 命中占比，便于多轮结果做纵向分析。
10. Qwen 与 Voyage 必须使用不同的 `PROJECT_SPACE`，否则会触发项目元数据锁定校验。
11. 如果希望扩大评测范围，优先新增新的 queries 文件版本，而不是直接改已有基准文件。
