...
1. 第一阶段先支持批处理，并以保守默认值运行
2. 当前实现已经支持固定并发度的批次执行，默认 `embeddingConcurrency = 1`
3. 在进一步提高默认并发前，先补齐重试、退避和限流策略
...
1. 继续将 `embeddingBatchSize` 和 `embeddingConcurrency` 暴露为运行时配置，并结合 provider 特性调优默认值
...
2. 当前实现支持固定大小并发池，默认按保守并发度调用 provider
3. 当真实环境的限流、超时和失败模式更明确后，再继续提高默认并发度或增加自适应调优
